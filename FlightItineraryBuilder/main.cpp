...
std::string format_time(int unix_time)
{
    ...
    #pragma warning(suppress : 4996)  // Suppresses the "unsafe" warning
...
int main() {
    ...
    if (!flight_manager.load_flight_data("all_flights.txt")) {
        std::cerr << "Failed to load flight data from " << "flight_data_file" << ".\n";
        return 1;
    }

    vector<FlightSegment> s = flight_manager.find_flights("HHR", 1736284400, 1736298800);

    for (int i = 0; i < s.size(); i++) {
        cout << s[i].source_airport << " " << s[i].destination_airport << " " << s[i].airline << " " << s[i].departure_time << endl;
    }
}
*/
